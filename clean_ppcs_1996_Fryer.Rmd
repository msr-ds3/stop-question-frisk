...
ppcs_1996_F <- ppcs_1996_F %>%
  ...
  mutate(any_force = ifelse((force_sum > 0) & !(is.na(force_sum) & had_contact == 1), 1, NA)) %>%
  mutate(any_force = ifelse(force_sum == 0, 0, any_force))
...
#argue
ppcs_1996_F <- ppcs_1996_F %>%
  mutate(civilian_argued = ifelse(YOU_DO_ARGUE_WITH_THE_OFFICER == 'Yes', 1, NA))
...
#any behavior
ppcs_1996_tmp <- ppcs_1996_F %>%
  select(civilian_threatened, civilian_physical, civilian_argued, civilian_disobey, civilian_other_behavior, civilian_escape, civilian_resist, civilian_has_weapon)
...
save(ppcs_1996_F, file = "clean_data/ppcs_1996_F.Rdata")
```
