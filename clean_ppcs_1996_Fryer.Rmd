...
```{r}
#civilian handcuffed - come back to this
ppcs_1996_F <- ppcs_1996_F %>% 
  mutate(officer_handcuffed = NA)
```
...
```{r}
# drop civilian has weapon, don't want to be confused with civilian guilty, only needed to get the row total above - from fryer code
...
ppcs_1996_F <- ppcs_1996_F %>%
  mutate(civilian_injured = case_when(
    ...
    (WERE_YOU_INJURED_AS_A_RESULT_OF_INCIDENT == 'missing/ni/ou' & police_used_force == 0) ~ 0
  ))
```
...
ppcs_1996_F <- ppcs_1996_F %>%
  mutate(excessive_force = case_when(
    ...
    (TAKE_ANY_FORMAL_OR_INFORMAL_ACTIONS == 'missing/ni/ou' & police_used_force == 0) ~ 0
  ))
```
