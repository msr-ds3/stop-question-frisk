...
ppcs_2011_F <- ppcs_2011_F %>%
  ...
  mutate(off_race = coalesce(off_race_1_s, off_race_1_t)) %>%
  ...
  mutate(civilian_other_behavior = coalesce(other_behavior_s, other_behavior_t)) %>%
  ...
  mutate(off_fired_gun = coalesce(fired_gun_s, fired_gun_t)) %>%
  mutate(off_shot_suspect = coalesce(shot_suspect_s, shot_suspect_t))
```
...
save(ppcs_2011_F, file = "clean_data/ppcs_2011_F.Rdata")
```
