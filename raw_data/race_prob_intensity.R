...
race_prob_low <- low_intensity %>%
  mutate(race = recode_factor(race,"P" = "B", "I" = "Z"), race = recode_factor(race, "W" = "White", "B" = "Black",  "Q" ="Hispanic",  "A" = "Asian", "Z" = "Other")) %>% 
  filter(pf_low == 1) %>% 
  mutate(tally = 1) %>%
  group_by(addrpct, race) %>% 
  summarize(total = sum(tally)) %>%
  ungroup() %>% group_by(addrpct) %>% 
  mutate(prop = total/sum(total)) %>% View()
  filter(prop == max(prop)) %>% 
 



race_prob_high <- high_intensity %>% filter(pf_high == 1) %>% 
  mutate(race = recode_factor(race,"P" = "B", "I" = "Z"), race = recode_factor(race, "W" = "White", "B" = "Black",  "Q" ="Hispanic",  "A" = "Asian", "Z" = "Other"))
  mutate(tally = 1) %>%
  group_by(addrpct, race) %>% 
  summarize(total = sum(tally)) %>%
