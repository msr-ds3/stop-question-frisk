...
```
```{r type-of-incident}
missing <- c('Not at all', 'missing/ni/ou')
...
ppcs_1996 <- ppcs_1996 %>%
  mutate(type_of_incident = case_when(
    ...
    (REASON_REPORT_A_CRIME %in% missing) ~ 3,
    (REASON_ASK_FOR_ASSISTANCE %in% missing) ~3,
    (REASON_PROBLEM_IN_THE_NEIGHBORHOOD %in% missing)~3,
    (REASON_VICTIM_OF_A_CRIME %in% missing) ~ 3,
    (REASON_WITNESS_TO_A_CRIME %in% missing) ~3,
    (REASON_CRIME_THOUGHT_INVOLVED_IN %in% missing) ~ 3,
    (REASON_WHAT_YOU_WERE_DOING_IN_THE_AREA %in% missing) ~3,
    (REASON_HAD_WARRANT_FOR_YOUR_ARREST %in% missing) ~3,
    (REASON_CASUAL_ENCOUNTERS %in% missing) ~ 3,
    (REASON_COMMUNITY_MEETINGS %in% missing) ~3,
    (REASON_SOME_OTHER_REASON %in% missing) ~3,
    (REASON_NO_SPECIFIC_REASON %in% missing) ~3,
    TRUE ~ NA_real_
  ))
```
