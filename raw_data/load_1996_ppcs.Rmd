...
# 
# dfcol <- data.frame(colnames(ppcs_1996)) %>% view
```
...
#View(ppcs_1996$REASON_TRAFFIC_PARKING_VIOLATION)
ppcs_1996 <- ppcs_1996 %>%
  mutate(type_of_incident = case_when(
    ...
    # (REASON_ASK_FOR_ASSISTANCE %in% happened) ~3,
    # (REASON_PROBLEM_IN_THE_NEIGHBORHOOD %in% happened)~3,
    # (REASON_VICTIM_OF_A_CRIME %in% happened) ~ 3,
    # (REASON_WITNESS_TO_A_CRIME %in% happened) ~3,
    (REASON_CRIME_THOUGHT_INVOLVED_IN %in% happened) ~ 3,
    # (REASON_WHAT_YOU_WERE_DOING_IN_THE_AREA %in% happened) ~3,
    # (REASON_HAD_WARRANT_FOR_YOUR_ARREST %in% happened) ~3,
    # (REASON_CASUAL_ENCOUNTERS %in% happened) ~ 3,
    (REASON_COMMUNITY_MEETINGS %in% happened) ~3,
    # (REASON_SOME_OTHER_REASON %in% happened) ~3,
    # (REASON_NO_SPECIFIC_REASON %in% happened) ~3,
    TRUE ~ NA_real_
  ))
```
