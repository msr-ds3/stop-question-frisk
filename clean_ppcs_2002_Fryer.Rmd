...
ppcs_2002_F <- ppcs_2002_F %>%
  mutate(type_of_incident = case_when(
    (TRAFFIC_ACCIDENT == "Yes" | TRAFFIC_STOP == "Yes") ~ 'traffic',
    (TRAFFIC_ACCIDENT == "No" & TRAFFIC_STOP == "No") ~ 'other',
    (TRAFFIC_ACCIDENT == "Out of universe/missing" &  had_contact == 1) ~ NA_character_
  ))
```
...
ppcs_2002_F <- ppcs_2002_F %>%
  mutate(force_sum = rowSums(ppcs_2002_tmp, na.rm = TRUE)) %>%
...
ppcs_2002_F <- ppcs_2002_F %>%
  mutate(behav_sum = rowSums(ppcs_2002_tmp, na.rm = TRUE)) %>%
  mutate(any_behavior = ifelse((behav_sum > 0) & (had_contact == 1), 1, 0))

ppcs_2002_F <- ppcs_2002_F %>%
  mutate_at(.funs = list( ~ ifelse(is.na(.) & ppcs_2002_F$any_force == 1, 0, .)), .vars = colnames(ppcs_2002_tmp)) %>%
  mutate_at(.funs = list( ~ ifelse(is.na(.) & POLICE_USED_OR_THREATENED_FORCE == 'No', 0, .)), .vars = colnames(ppcs_2002_tmp))
```
...
save(ppcs_2002_F, file = "clean_data/ppcs_2002_F.Rdata")
```
