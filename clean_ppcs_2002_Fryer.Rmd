...
ppcs_2002_F <- ppcs_2002_F %>%
    mutate(population_size = case_when(
      (SIZE_OF_JURISDICTION_WHERE_RESPONDENT_RESIDED == "Under 100,000/not in a place") ~ 1,                                                (SIZE_OF_JURISDICTION_WHERE_RESPONDENT_RESIDED == "100,000-499,999") ~ 2,
...
```{r}
#Interesting: theres is another column called POLICE_USED_OR_THREATENED_OTHER_FORCE which isn't used. In 1999 used both for traffic and other
# police used force
ppcs_2002_F <- ppcs_2002_F %>%
...
ppcs_2002_F <- ppcs_2002_F %>%
  mutate(civilian_disobey = ifelse(TRAFFIC_STOP_OTHER_TYPE_OF_COMPLAINT_FILED == 'Yes', 1, NA))
