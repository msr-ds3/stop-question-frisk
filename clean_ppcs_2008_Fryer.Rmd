...
ppcs_2008_F <- ppcs_2008_F %>%
  mutate(hit_kick = ifelse(is.na(hit_kick) & police_used_force == 0, 0, hit_kick)) %>%
  mutate(grab_push = ifelse(is.na(grab_push) & police_used_force == 0, 0, grab_push)) %>%
  mutate(point_gun = ifelse(is.na(point_gun) & police_used_force == 0, 0, point_gun)) %>%
  mutate(pepper_stun = ifelse(is.na(pepper_stun) & police_used_force == 0, 0, pepper_stun)) %>%
  mutate(other_force = ifelse(is.na(other_force) & police_used_force == 0, 0, other_force))
...
ppcs_2008_F <- ppcs_2008_F %>%
  ...
  mutate(civilian_injured = ifelse(is.na(civilian_injured) & police_used_force == 0, 0, civilian_injured ))
...
ppcs_2005_F <- ppcs_2005_F %>%
  mutate_at(.funs = list( ~ ifelse(is.na(.) & ppcs_2005_F$police_used_force == 0, 0, .)), .vars = c("hit_kick", "grab_push", "point_gun", "pepper_stun", "other_force"))
...
#officer shot suspect
ppcs_2008_F <- ppcs_2008_F %>%
  mutate(off_shot_suspect = case_when(
    (V8A1 == 1) ~ 1
  ))
```
...
save(ppcs_2008_F, file = "clean_data/ppcs_2008_F.Rdata")
```
